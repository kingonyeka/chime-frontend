...
const ForexTradingProduct = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const formattedData = data.data.filter(course => course.type_id.includes("forex-trading"));
...
const Pagination = ({ coursesPerPage, totalCourses, paginate, currentPage }) => {
  const pageNumbers = Array.from({ length: Math.ceil(totalCourses / coursesPerPage) }, (_, i) => i + 1);
...
export default ForexTradingProduct;
